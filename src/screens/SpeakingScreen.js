...
const autoSoundSaverInstance = AutoSoundsSaverObj
...
export default class TextToSpeachScreen extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      text: "",
      predectedWords: TextPredictionObj.getPredectedWords(""),
      textWidth: '99%',
      toolsBar: [
        {
          title: "نطق",
  ...
  selectPredectedWord(word) {
    ...
    logEvent(EVENTS.SELECT_PREDICTED_WORD, {
      text: word
    });
  }
  ...
  changeText(text) {
    ...
    toolsColors["مشاركة"] =  text.trim().length > 0 ? Colors.brand
    : Colors.borderColor;
    ...
    this.setState({
      ...
      predectedWords: TextPredictionObj.getPredectedWords(text) // .slice(0, 12)
    });
  }
  ...
  speak() {
    ...
    TextPredictionObj.addSentenceToUserWords(this.state.text);
    TextToSpeachObj.speak(this.state.text);
    setTimeout(() => {
      toolsBar[0].styles = defaultStyles;
      this.setState({ toolsBar: toolsBar });
    }, (this.state.text.length / 10.5) * 1000);
    logEvent(EVENTS.SPEAK, {
      text: this.state.text,
    });
  }
  ...
  deleteFavourite() {
    ...
    if (currFavourites.includes(this.state.text)) {
      ...
      StorageObj.setItem("favourites", currFavourites).then(res => {});
    }
    ...
    logEvent(EVENTS.DELETE_FAVORITE, {
      text: this.state.text,
    });
  }
  ...
  toggleFavourites() {
    ...
    if (toolsColors["المفضلة"] == Colors.brand) {
      ...
      if (!currFavourites.includes(this.state.text)) {
        ...
        autoSoundSaverInstance.storeSoundIfNotExist(this.state.text);
        StorageObj.setItem("favourites", currFavourites).then(res => {});
